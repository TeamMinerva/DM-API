...
const RegionalParticipationCard: React.FC<RegionalParticipationCardProps> = ({
  ...
  return (
    <div
      className={`w-full h-full bg-[#F1EFFF] rounded-[20px] p-6 font-[Catamaran] flex flex-col transition-opacity duration-500 ${
    ...
    >
      ...
      {loading && (
        <div className="mt-9 text-sm font-semibold text-[#7B7E86]">
          Carregando dados...
        </div>
      )}

      {error && !loading && (
        <div className="mt-9 text-sm font-semibold text-red-500">{error}</div>
      )}
